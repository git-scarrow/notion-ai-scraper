...
1. If `Canon Review Consumed At` is already set, **HALT without writing**. Duplicate trigger.
1. If `Canon Review Requested At` is empty, **HALT without writing**. Spurious trigger.
1. **FIRST WRITE:** Set `Canon Review Consumed At = now()` and `Pipeline Status = Canon Review`.
## Trigger Contract — Revision Pass
Fires when `Scene Revision Requested At` changes on `pontius_scene_items`.
**Consume-first idempotency:**
1. Read the triggering Scene Item.
1. If `Scene Revision Consumed At` is already set, **HALT without writing**. Duplicate trigger.
1. If `Scene Revision Requested At` is empty, **HALT without writing**. Spurious trigger.
1. **FIRST WRITE:** Set `Scene Revision Consumed At = now()` and `Pipeline Status = Canon Review`.
...
1. Identify failure modes to watch (from ANTI_FLATTENING) specific to the primary type.
1. **Escalation check:** If this is the FIRST occurrence of a cognitive-rhetorical schema (eschatological, forensic, exegetical) being activated in a new season:
...
1. Verify all four fields are respected:
...
1. Set `Escalation Level` to `Needs Sam` (or `Critical` for character_death)
1. Set `Blocked Reason` with a specific, actionable description
1. Set `Pipeline Status = Escalated`
1. Do NOT stamp `Dramatic Architecture Requested At` or `Human Review Requested At`
1. HALT.
