...
1. If `Source Grounding Consumed At` is already set, **HALT without writing**. This is a duplicate trigger; the signal was already processed.
1. If `Source Grounding Requested At` is empty, **HALT without writing**. Spurious trigger.
1. **FIRST WRITE:** Set `Source Grounding Consumed At = now()` and `Pipeline Status = Source Grounding`. This blocks re-entry before you do any work.
...
1. **Source retrieval.** Query the RAG corpus (Oracle backend) for primary sources relevant to the scene. Search by:
...
1. **Provenance tagging.** Classify every source per the Charter's Three Pillars:
...
1. **Evidence grading.** For each finding, assign a burden level per RESEARCH_PROTOCOL:
...
1. **Gap identification.** For scene elements with no source backing, document the gap and the reason. Do NOT invent historical detail to fill gaps — that is the Dramatic Architect's job in Step 6.
1. **Escalation check.** If the creative brief requires placing a character at a date or location not supported by independent sources:
  ...
  - HALT.
1. **Write outputs** to the Scene Item:
...
1. **Handoff.** If `Task Type` is `Research Query`:
